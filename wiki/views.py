...
def view_version(request, slug, version):
    page = get_object_or_404(Page, key_name="page%s%s" % (slug, version))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_page = Page.objects.create_with_auto_version(slug, cd['headline'], cd['content'],
                cd['change_message'], request.META['REMOTE_ADDR'], cd['minor_edit'])
...
def history(request, slug):
    page_list = Page.objects.order_by('-version').filter('slug =', slug)
    if not page_list:
